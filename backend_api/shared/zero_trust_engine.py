...
class Identity(BaseModel):
    ...
    attributes: Dict[str, Any] = Field(
        {}, description="Attributes associated with the identity"
    )
...
class AccessRequest(BaseModel):
    request_id: str = Field(
        ...
        description="Unique ID for the access request",
    )
    ...
    action: str = Field(
        ...,
        description="The action being performed on the resource (e.g., 'read', 'write', 'execute')",
    )
    context: Dict[str, Any] = Field(
        {},
        description="Contextual information (e.g., 'source_ip', 'time_of_day', 'device_health')",
    )
...
class TrustScore(BaseModel):
    ...
    score: float = Field(
        ..., ge=0, le=100, description="Calculated trust score (0-100)"
    )
    factors: Dict[str, Any] = Field(
        {}, description="Factors influencing the trust score"
    )
    last_evaluated: float = Field(
        default_factory=time.time,
        description="Timestamp of the last trust score evaluation",
    )
...
class ZeroTrustPolicy(BaseModel):
    ...
    rules: List[str] = Field(
        ...,
        description="List of policy rules (e.g., 'allow if user_trust_score > 80 and device_health = high')",
    )
    enforcement_action: str = Field(
        "deny",
        description="Action if policy is violated ('deny', 'alert', 'quarantine')",
    )
...
class EnforcementAction(BaseModel):
    action_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()),
        description="Unique ID for the enforcement action",
    )
    access_request_id: str = Field(
        ..., description="ID of the access request that triggered the action"
    )
    policy_id: str = Field(..., description="ID of the violated policy")
    enforced_action: str = Field(
        ..., description="The action taken (e.g., 'denied_access', 'isolated_device')"
    )
    timestamp: float = Field(
        default_factory=time.time, description="Timestamp of the enforcement"
    )
    details: Dict[str, Any] = Field(
        {}, description="Additional details about the enforcement"
    )
...
class ZeroTrustEngine:
    ...
    def add_policy(self, policy: ZeroTrustPolicy):
        ...
        logger.info(
            f"[{__name__}] Added Zero-Trust policy: {policy.name} ({policy.policy_id})"
        )

    def update_policy(
        self, policy_id: str, updated_policy: ZeroTrustPolicy
    ) -> Optional[ZeroTrustPolicy]:
        """Updates an existing Zero-Trust policy."""
        if policy_id in self.policies:
            self.policies[policy_id] = updated_policy
            logger.info(
                f"[{__name__}] Updated Zero-Trust policy: {updated_policy.name} ({policy_id})"
            )
    ...
    async def evaluate_access_request(
        ...
        logger.info(
            f"[{__name__}] Evaluating access request (ID: {request.request_id}) for {request.identity.id} accessing {request.resource}"
        )

        trust_score_obj = await self.get_trust_score(request.identity.id)
        if not trust_score_obj:
            trust_score_obj = TrustScore(
                entity_id=request.identity.id,
                score=0,
        ...
        for policy in self.policies.values():
            ...
            for rule in policy.rules:
                if not self._safe_evaluate_rule(rule, context):
            ...
            if policy_violated:
                ...
                logger.warning(
                    f"[{__name__}] Access request {request.request_id} violated policy {policy.name}. Action: {enforced_action}"
                )
        ...
        enforcement_log = EnforcementAction(
            ...
            details={
                "access_granted": enforced_action == "allowed",
                "trust_score_at_request": trust_score_obj.score,
            },
        )
        self.enforcement_logs.append(enforcement_log)
        return enforcement_log

    def _safe_evaluate_rule(self, rule: str, context: Dict[str, Any]) -> bool:
        """
        Safely evaluates a simple rule string against the request context.
        Supports operators: >, <, =, !=
        Example: 'user_trust_score > 80'
        """
        try:
            parts = rule.split()
            if len(parts) != 3:
            ...
            variable_name, operator, value_str = parts
            
            if variable_name not in context:
                logger.warning(f"Variable '{variable_name}' from rule not in request context. Skipping rule.")
                return False
            ...
            if isinstance(context_value, (int, float)):
                rule_value = float(value_str)
            elif isinstance(context_value, str):
                # The value in the rule might be quoted, remove quotes
                rule_value = value_str.strip("'\"")
            else:
            ...
            if operator == '>':
                ...
                logger.warning(f"Unsupported operator '{operator}' in rule: '{rule}'")
                return False
            ...
            logger.error(f"An unexpected error occurred while evaluating rule '{rule}': {e}")
            return False
        return True
    ...
    async def test_zero_trust_engine():
        ...
        new_policy = ZeroTrustPolicy(
            ...
            rules=[
                ...
                "resource_path.startswith('/restricted')",
            ],
            enforcement_action="deny",
        )
        ...
        identity_user = Identity(
            id="user123", type="user", attributes={"username": "alice"}
        )
        ...
        (
            print(json.dumps(trust_score.dict(), indent=2))
            if trust_score
            else "Trust score not found."
        )
