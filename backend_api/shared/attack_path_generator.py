...
class AttackPathNode(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Display name of the node (e.g., Asset Name, Vulnerability Name)",
    )
    type: str = Field(
        ...,
        description="Type of node (e.g., 'asset', 'vulnerability', 'exploit', 'entry_point')",
    )
    risk_score: Optional[int] = Field(
        None, ge=0, le=100, description="Risk score associated with the node"
    )
    description: Optional[str] = Field(
        None, description="Detailed description of the node"
    )
...
class AttackPathEdge(BaseModel):
    ...
    relationship: str = Field(
        ...,
        description="Description of the relationship (e.g., 'exploits', 'has_vulnerability', 'leads_to')",
    )
    risk_factor: Optional[float] = Field(
        None, ge=0, le=1.0, description="Factor contributing to overall path risk"
    )
...
class AttackPathGraph(BaseModel):
    ...
    overall_risk: Optional[float] = Field(
        None, ge=0, le=1.0, description="Calculated overall risk of this path"
    )
...
def generate_simulated_attack_path(depth: int = 3) -> AttackPathGraph:
        ...
        for c_node in current_nodes:
            ...
            if c_node.type in ["exploit", "asset", "entry_point"]:
                ...
                asset_type = random.choice(
                    ["WebServer", "Database", "Internal Server", "Workstation"]
                )
                ...
                edges.append(
                    AttackPathEdge(
                        source=c_node.id,
                        target=asset_node.id,
                        relationship=random.choice(
                            ["leads_to", "accesses", "pivot_to"]
                        ),
                    )
                )
    ...
    overall_risk = (
        sum([n.risk_score for n in nodes if n.risk_score is not None]) / len(nodes)
        if nodes
        else 0
    )
    ...
    return AttackPathGraph(
        ...
        edges=edges,
        summary=summary,
        overall_risk=round(overall_risk / 100, 2),
    )
