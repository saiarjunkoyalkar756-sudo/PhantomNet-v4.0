...
from .database import SessionLocal, User, SessionToken, AttackLog, Agent
import random
...
async def check_api_gateway_health():
    try:
        ...
        logger.error(
            "API Gateway health check: FAILED", status="unhealthy", error=str(e)
        )
        # Metrics: Increment api_gateway_health_status_failed_total
        return False
...
async def check_jwt_expiry_anomalies():
    try:
        ...
        if expired_token_count > 100:
            logger.warning(
                "Mass token expiry detected", expired_token_count=expired_token_count
            )
        ...
        for agent in agents:
            ...
            if peer and peer.id != agent.id:
                try:
                    ...
                    logger.error(
                        "Error gossiping between agents",
                        agent_id=agent.id,
                        peer_id=peer.id,
                        error=str(e),
                    )
...
async def check_key_rotation():
    try:
        ...
        for agent in agents:
            ...
            if agent.last_seen and (datetime.utcnow() - agent.last_seen) > timedelta(
                days=90
            ):
                ...
                try:
                    async with httpx.AsyncClient() as client:
                        response = await client.post(
                            f"http://localhost:8000/api/agents/{agent.id}/rotate-key"
                        )
                        response.raise_for_status()
                        logger.info(
                            "Agent key rotation triggered successfully",
                            agent_id=agent.id,
                        )
                    ...
                    logger.error(
                        "Error triggering agent key rotation",
                        agent_id=agent.id,
                        error=str(e),
                    )
...
async def monitor_health(interval: int = 60):
    while True:
        ...
        if not other_services_healthy:
            logger.warning(
                "One or more other services are unhealthy. Simulating restart..."
            )
