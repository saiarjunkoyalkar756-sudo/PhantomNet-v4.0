...
API_GATEWAY_URL = (
    "http://api_gateway:8000"  # Assuming api_gateway is accessible via this hostname
)
...
def check_abuseipdb_sync(ip_address: str, log_id: int):
    ...
    try:
        ...
        ):  # Threshold for considering it a verified threat
            print(
                f" [Analyzer] IP {ip_address} is a verified threat (AbuseIPDB score: {data['data']['abuseConfidenceScore']})"
            )
        ...
        try:
            attack_log_entry = (
                db.query(AttackLog).filter(AttackLog.id == log_id).first()
            )
            if attack_log_entry:
                ...
                print(
                    f" [Analyzer] Updated AttackLog ID {log_id} with AbuseIPDB verification status."
                )
                if is_verified_threat:
                    # Trigger admin alert for verified threat
                    httpx.post(
                        f"{API_GATEWAY_URL}/alerts/threat_verified",
                        json={
                            "log_id": log_id,
                            "ip": ip_address,
        ...
        print(
            f" [Analyzer] Unexpected error during AbuseIPDB check for {ip_address}: {e}"
        )
...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        print(
            f" [Analyzer] Predicted attack type: {prediction} with confidence: {confidence_score:.2f}"
        )

        # Perform anomaly detection
        anomaly_score = anomaly_model.decision_function(features_df)[0]
        is_anomaly = (
            anomaly_model.predict(features_df)[0] == -1
        ...
        if is_anomaly:
            ...
            try:
                httpx.post(
                    f"{API_GATEWAY_URL}/alerts/anomaly",
                    json={
                        "log_id": log_id,
        ...
        try:
            ...
            if attack_log_entry:
                ...
                db.refresh(attack_log_entry)
                print(
                    f" [Analyzer] Updated AttackLog ID {log_id} with prediction, anomaly status, and geolocation."
                )
        ...
        if original_log_data["ip"]:
            thread = threading.Thread(
                target=check_abuseipdb_sync, args=(original_log_data["ip"], log_id)
            )
            thread.start()
        ...
        try:
            ...
            recent_attacks = (
                db_for_blacklist.query(AttackLog)
                .filter(
            ...
            if recent_attacks >= 3:
                print(
                    f" [Analyzer] Repeated attacks from {original_log_data["ip"]}. Mock blacklisting."
                )
                # Update all recent attacks from this IP as blacklisted
                db_for_blacklist.query(AttackLog).filter(
                    AttackLog.ip == original_log_data["ip"],
                    AttackLog.timestamp >= five_minutes_ago,
                ...
                httpx.post(
                    f"{API_GATEWAY_URL}/alerts/blacklisted",
                    json={
                        "ip": original_log_data["ip"],
    ...
    channel.basic_consume(
        queue="attack_logs", on_message_callback=callback, auto_ack=True
    )
