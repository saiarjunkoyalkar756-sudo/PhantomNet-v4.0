...
@app.api_route("/rules", methods=["GET", "POST", "PUT", "PATCH", "DELETE"], include_in_schema=False)
@app.api_route("/rules/{legacy_path:path}", methods=["GET", "POST", "PUT", "PATCH", "DELETE"], include_in_schema=False)
async def retired_legacy_rule_api(legacy_path: str = ""):
    """Block the legacy non-tenant-scoped rule API instead of retaining an authorization bypass."""
    return error_response(
        code="LEGACY_RULE_API_RETIRED",
        message="The legacy rule API is retired. Use tenant-scoped /governed-rules routes with the required capability.",
        status_code=410,
    )
