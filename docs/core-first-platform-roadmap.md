...
## Completed bounded increment: evidence-first event and case analyst workspaces

The event-stream and case-management pages now use the SOC console's operator-focused information architecture. The event workspace exposes evidence-source posture, transport and browser-storage boundaries, server-side filter constraints, an intentionally empty authorization-gated observation workbench, and evidence guardrails. The case workspace exposes governed lifecycle posture, an intentionally empty tenant-scoped queue, disabled direct case creation, and the alert → analyst triage → playbook request → verification and rollback control sequence.

Both pages preserve their existing fail-closed product boundaries. The event view does not open a WebSocket, retain a browser event feed, construct raw telemetry queries, or gain response authority. The case view does not list, create, assign, annotate, transition, close, or execute a playbook through the retired legacy case surface. The supported governed case lifecycle remains separately protected and must retain authenticated tenant scope, analyst capabilities, auditable state transitions, approval-bound non-executing playbooks, and the separate request → human approval → HMAC-signed audit → controlled execution → verification → rollback response lifecycle.

> **Evidence boundary:** this is Class A source, lint, build, and local visual-preview evidence. It does not prove tenant data access, event collection, source provenance in a deployment, alert ingestion, incident-case behavior, analyst authorization in a live session, real playbook operation, response execution, containment, audit durability, verification, rollback, responsive operation on a user device, Docker-host operation, or defensive efficacy.


## Completed bounded increment: evidence-first threat-hunting detail workspace

The active tenant-scoped threat-hunting page has been rebuilt using the SOC console system while preserving its supported backend contract. The workspace now presents the governed summary as source-dependent posture cards, an allowlisted structured-hunt builder, read-only automated template summaries, saved-hunt reuse, returned-evidence tables, summary charts that remain withheld without returned counts, and a collapsible bounded graph-analysis workspace.

The active hunt, saved-hunt, dashboard-summary, automated-template, and governed attack-path clients remain. The page does not accept a raw query language, invent summary counts when the service is unavailable, represent graph traversal as free-form querying, or grant hunt, template, graph, AI, or dashboard controls response authority. A hunt remains read-only and separately bounded graph analysis remains tenant-scoped and non-executing.

> **Evidence boundary:** this is Class A source, lint, build, and local visual-preview evidence. It does not prove authenticated live-session access, tenant data access, event collection, query-result correctness, alert volume, chart accuracy, saved-hunt persistence, graph projection, attack-path analysis, analyst authorization, containment, response execution, audit durability, verification, rollback, responsive operation on a user device, Docker-host operation, or defensive efficacy.


## Completed bounded increment: governed incident-response and advisory-triage workspace

The retired fixture-based SOAR page has been replaced with an operator workspace mapped only to the active governed containment and durable advisory-decision API contract. It exposes scoped request creation, a tenant request ledger, server-provided side-effect-free preflight posture, reason-bearing human decision recording, eligibility-gated execution and rollback controls, signed-audit-chain verification, and an advisory evaluation panel for a tenant-owned detection.

The interface does not recreate legacy playbook CRUD, fixture approvals, local execution history, automatic mitigation, or browser-generated decision outcomes. A playbook reference is request metadata only. Every high-impact operation remains server-authorized and constrained by the request → human decision → HMAC-signed audit → adapter preflight → controlled execution → verification → rollback lifecycle. Automated triage can return durable policy outcomes or an approval-bound proposal, but it cannot execute an adapter or self-approve containment.

> **Evidence boundary:** this is Class A source, lint, build, and local visual-preview evidence. It does not prove authenticated live-session access, capabilities, tenant data access, request persistence, audit configuration, audit-chain validity, human approval behavior, adapter eligibility, external adapter dispatch, Wazuh or cloud integration, execution, verification, rollback, advisory accuracy, autonomous-defense policy quality, responsive operation on a user device, Docker-host operation, or defensive efficacy.
