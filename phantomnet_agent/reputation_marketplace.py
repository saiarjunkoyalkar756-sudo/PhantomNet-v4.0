...
class NodeReputation:
    ...
    def __init__(self, node_id: str, initial_score: float = 0.5):
        ...
        print(
            f"Node {self.node_id} reputation initialized with score: {self.reputation_score:.2f}"
        )
    ...
    def add_contribution(self, contribution_type: str, quality_score: float):
        ...
        self.reputation_score = max(
            0.0, min(1.0, self.reputation_score + (quality_score - 0.5) * 0.1)
        )  # Adjust based on quality
        print(
            f"Node {self.node_id} added {contribution_type} (quality: {quality_score:.2f}). New reputation: {self.reputation_score:.2f}"
        )
    ...
    def store_on_chain(self):
        ...
        """
        print(
            f"Simulating storing reputation for Node {self.node_id} ({self.reputation_score:.2f}) on-chain."
        )
...
class TokenomicsEngine:
    ...
    def calculate_reward(self, reputation_score: float) -> float:
        ...
        print(
            f"Calculated reward for reputation {reputation_score:.2f}: {reward:.2f} tokens."
        )
        return reward
