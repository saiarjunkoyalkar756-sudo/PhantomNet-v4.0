...
class BlueTeamAgent:
    ...
    def generate_action(self, current_state: dict, red_action: str) -> str:
        ...
        """
        print(
            f"[{self.name}] Red action observed: {red_action}. Analyzing state: {current_state}"
        )
...
class SimulationEnvironment:
    ...
    def update_state(self, red_action: str, blue_action: str):
        ...
        """
        print(
            f"[Environment] Red performed: {red_action}, Blue performed: {blue_action}"
        )
        ...
        if "brute_force_ssh" in red_action and "block_ip" in blue_action:
            ...
            "exploit_vulnerability" in red_action
            and "patch_vulnerability" in blue_action
        ):
            self.current_state["alerts"].append("Vulnerability patched.")
        else:
            self.current_state["alerts"].append(
                f"Action: {red_action} vs {blue_action}"
            )

        # Simulate some state changes
        self.current_state["network_status"] = random.choice(
            ["normal", "elevated_alert"]
        )
    ...
    def run_simulation(self, num_steps: int = 10):
        ...
        for step in range(num_steps):
            ...
            blue_action = self.blue_agent.generate_action(
                self.current_state, red_action
            )
