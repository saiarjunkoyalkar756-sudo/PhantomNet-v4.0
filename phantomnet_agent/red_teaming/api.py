...
async def start_redteam_run(
    ...
    try:
        run = orchestrator.schedule_run(
            playbook_id, instance_id, mode, operator_id["username"]
        )
...
async def get_redteam_run_status(
...
async def get_redteam_run_report(
    run_id: str, current_user: dict = Depends(get_current_user)
):
...
async def add_playbook(
    playbook: Playbook, current_user: dict = Depends(get_current_user)
):
    try:
