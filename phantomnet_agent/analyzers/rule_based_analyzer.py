...
class RuleBasedAnalyzer(Analyzer):
    def __init__(self):
        self.rules = {
            "SQL Injection": r"(?:\b(select|union|insert|update|delete|drop)\b|['\"]\s*or\s*['\"]?\d)",
            "XSS": r"(?:<script\b|<img\b[^>]*\bonerror\s*=|javascript:)",
            "Directory Traversal": r"(?:\.\./|\\.\\\\|%2e%2e%2f|%2e%2e%5c)",
            "Port Scan": r"\b(nmap|masscan|zmap)\b",
            "Brute Force": r"(?:\b(login|password|username|user|pass|auth|access|admin)\b|\b\w+\s*:\s*\w+\b)",
        }
