...
def test_legacy_api_gateway_defers_to_shared_environment_scoped_cors_policy():
    module = ast.parse(GATEWAY_SOURCE.read_text(encoding="utf-8"), filename=str(GATEWAY_SOURCE))
    factory_calls = [
        node
    ...
    keywords = {keyword.arg: keyword.value for keyword in factory_calls[0].keywords if keyword.arg}
    assert "cors_origins" not in keywords
    assert '"*"' not in GATEWAY_SOURCE.read_text(encoding="utf-8")
