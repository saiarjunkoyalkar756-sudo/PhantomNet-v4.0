...
class ComplianceFinding(BaseModel):
    ...
    status: str = Field(..., description="Compliance status ('compliant', 'non-compliant', 'n/a')")
    evidence: Optional[str] = Field(None, description="Evidence supporting the status")
    severity: str = Field("medium", description="Severity of non-compliance ('low', 'medium', 'high', 'critical')")

class ComplianceHeatmap(BaseModel):
    control_area: str = Field(..., description="High-level control area (e.g., 'Access Control', 'Data Protection')")
    compliant_percentage: float = Field(..., ge=0, le=100, description="Percentage of compliant controls in this area")
    non_compliant_count: int = Field(..., ge=0, description="Number of non-compliant controls")
...
class ImprovementPlanRecommendation(BaseModel):
    ...
    ai_rationale: Optional[str] = Field(None, description="AI-generated rationale for the recommendation")

class ComplianceReport(BaseModel):
    report_id: str = Field(..., description="Unique ID for the report")
    standard: str = Field(..., description="Compliance standard (e.g., 'ISO27001', 'SOC2')")
    timestamp: float = Field(default_factory=time.time, description="Timestamp of report generation")
    overall_status: str = Field(..., description="Overall compliance status ('compliant', 'non-compliant', 'partial')")
    findings: List[ComplianceFinding] = Field([], description="List of individual compliance findings")
    heatmap: List[ComplianceHeatmap] = Field([], description="Summary heatmap data")
    gap_analysis: List[ComplianceFinding] = Field([], description="List of non-compliant findings (gaps)")
    ai_improvement_plan: List[ImprovementPlanRecommendation] = Field([], description="AI-generated improvement plan")

class ComplianceEngine:
    def __init__(self):
        self.reports: Dict[str, ComplianceReport] = {} # Store generated reports

    async def _simulate_compliance_check(self, standard: str) -> List[ComplianceFinding]:
        ...
        for i in range(num_findings):
            ...
            findings.append(ComplianceFinding(
                ...
                evidence=f"Log files, configuration, policy doc {random.randint(100,999)}" if status_choice == "compliant" else None,
                severity=severity_choice
            ))
        return findings

    async def _generate_heatmap(self, findings: List[ComplianceFinding]) -> List[ComplianceHeatmap]:
        """Simulates generating a compliance heatmap."""
        time.sleep(random.uniform(0.5, 1.5))
        control_areas = ["Access Control", "Data Protection", "Incident Response", "Risk Management", "Security Operations"]
        heatmap_data = []
        for area in control_areas:
            ...
            heatmap_data.append(ComplianceHeatmap(
                control_area=area,
    ...
    async def _generate_improvement_plan(self, gap_analysis: List[ComplianceFinding]) -> List[ImprovementPlanRecommendation]:
        ...
        for finding in gap_analysis:
            if finding.status == "non-compliant":
                ...
                recommendations.append(ImprovementPlanRecommendation(
                    ...
                    ai_rationale="Identified as a critical vulnerability affecting data integrity and confidentiality. Prompt remediation is recommended." if priority == "high" else None
                ))
    ...
    async def run_compliance_scan(self, standard: str) -> ComplianceReport:
        ...
        logger.info(f"[{__name__}] Starting compliance scan for {standard} (Report ID: {report_id})")
        ...
        logger.info(f"[{__name__}] Completed compliance scan for {standard}. Report ID: {report_id}, Status: {overall_status}")
        return report
...
if __name__ == "__main__":
    ...
    async def test_compliance_engine():
        ...
        print(json.dumps(retrieved_iso.dict(), indent=2)) if retrieved_iso else "Report not found."
