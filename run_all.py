...
def stop_services():
    ...
    for pid_str in pids:
        try:
            ...
            print(f"Sent SIGTERM to process group with PGID {pid}")
...
def deploy_phantomnet():
    os.environ["PYTHONPATH"] = os.path.abspath(PROJECT_ROOT)
    check_prerequisites()
    ...
    # The original database.py script was moved.
    # The new way to initialize the database is by calling the create_db_and_tables function
    # from the shared.database module.
    # We will call it for the 'operational' database.
    run_command([sys.executable, "-c", "from backend_api.shared.database import create_db_and_tables; create_db_and_tables('operational')"])
    ...
    backend_process = start_process_background(
        [
            ...
            "-m",
            "uvicorn",
            "backend_api.api_gateway.app:app",
            "--host",
            "0.0.0.0",
            "--port",
            "8000",
